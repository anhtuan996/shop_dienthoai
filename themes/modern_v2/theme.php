...
{
	global $home, $array_mod_title, $lang_global, $language_array, $global_config, $site_mods, $module_name, $module_file, $module_data, $module_info, $op, $mod_title, $my_head, $my_footer, $client_info, $user_info;
	...
	{
		$xtpl->assign( 'THEME_LOGIN_INDEX_HREF', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=users&" . NV_OP_VARIABLE . "=login" );
		$xtpl->assign( 'THEME_REGISTER_INDEX_HREF', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=users&" . NV_OP_VARIABLE . "=register" );
        $xtpl->parse( 'main.nouser' );
	}
	else
	{
		$xtpl->assign( 'THEME_LOGOUT_INDEX_HREF', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=users&" . NV_OP_VARIABLE . "=logout" );
		$xtpl->assign( 'THEME_USERINFO_INDEX_HREF', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=users&" . NV_OP_VARIABLE . "=main" );
	...
	//menu footer
	$sql = "SELECT catid, parentid, title, alias, numsubcat, subcatid FROM `" . NV_PREFIXLANG . "_news_cat` WHERE lev IN (0,1) ORDER BY `order` ASC";
	$list = nv_db_cache( $sql, 'catid', 'news' );
	if( ! empty( $list ) )
	{
		$link = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=news&" . NV_OP_VARIABLE . "=";
		$i = 0;
		foreach( $list as $cats )
		{
			if( $cats['parentid'] == 0 )
			{
				$cats['link'] = $link . $cats['alias'];
				$i++;
				if( $cats['numsubcat'] > 0 )
				{
					$cats['subcatid'] = explode( ",", $cats['subcatid'] );
					foreach( $list as $cats_i )
					{
						if( in_array( $cats_i['catid'], $cats['subcatid'] ) )
						{
							$cats_i['link'] = $link . $cats_i['alias'];
							$xtpl->assign( 'SUBCAT', $cats_i );
							$xtpl->parse( 'main.cat.subloop.loop' );
						}
					}
					$xtpl->parse( 'main.cat.subloop' );
